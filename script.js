...
gridSizeBtn.addEventListener("click", () => {
    let gridSize = Number(
        window.prompt("What size of grid do you want? (Min 2, Max 100)", 16)
    );
    ...
    while (true) {
        if (gridSize == 0) {
            ...
            gridSize = Number(
                window.prompt(
                    "Chosen size was either too big or too small. What size of grid do you want? (Min 2, Max 100)",
                    16
                )
            );
            continue;
    ...
    gridBoxes.forEach((item) => {
        item.addEventListener("mouseover", function (e) {
            if (mode == "Standard") {
                ...
                e.target.style.background = incrementBlack(
